...
const FACTS: { fact: string; why: string }[] = [
    ...
    { fact: "handed back to `remove_path` as its own `tiles` it lifts exactly those, so a run laid wrong is not permanent", why: "the undo now sits in the build recipe beside the build_path that lays the run, because a run of three failing build_path retries is where the model needed it and Traps is not where it was looking; both tools take the same field under the same name, so the undo is a copy rather than a reconstruction" },
    { fact: "It paves the tiles listed in `tiles` and no others: no line is filled in between them, nothing is added to reach anything, and the order is left alone", why: "build_path took two endpoints and routed between them, so the shape of every path in the park was chosen by a search in pathbuild.ts rather than by the model - the same defect find_build_sites had. A model that expects a router draws two tiles and waits for the line" },
    { fact: "those two runs cannot share a tile", why: "the failure this was rebuilt from, watched in a live run: the router picked the queue's line and the exit path's line independently, the queue was laid over part of the exit's path, the ride claimed the whole chain and the exit was orphaned" },
    { fact: "A queue laid onto another ride's queue is not refused: the game chains the two lines into one", why: "the one kind of queue damage build_path allows through, because the game allows it; the refusal covers ordinary path over a queue and nothing else can" },
