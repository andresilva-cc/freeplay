...
export class MapViewTools {
    @mcpTool({
        ...
        description: [
            ...
            "It reads the ground: what each tile is, and what stands on it. What joins what is",
            "`park_status`'s `paths`, which measures the walkable network rather than the ground.",
            ...
            "A TAG IS BUILT UP, NOT PICKED OFF A LIST. A leading `U` means the park does not own that",
            "ground; what follows says what the ground is, and a trailing `S` means scenery is standing",
            "on it. So `E` is the park's, flat and empty; `S` the park's, flat, with scenery",
            "`clear_scenery` takes down; `^` the park's but sloped, which no typed tool here levels",
            "though `evaluate` reaches the game's own landsetheight, landraise, landlower and landsmooth;",
            "`^S` sloped WITH scenery on it - a path can still be built on a slope, so that scenery is in",
            "the way of something; `~` water; and `UE`, `US`, `U^`, `U^S`, `U~` are those same five on",
            "ground the park does not own, which `buy_land` is for.",
            "Things standing on a tile come first and hide the ground under them: `G` the park's gate,",
            "`N` a ride's entrance building, `X` a ride's exit building, `r` and a number for track",
            "belonging to that ride - `r3` is the ride `park_status` reports as id 3 - `Q` queue and `P`",
            "footpath, with `UQ` and `UP` those two on land the park does not own. A scenario's entrance",
            "corridor reads as `UP`: guests walk it, and it is not the park's land, so `build_path` refuses",
            "it and `buy_land` gets it only if the scenario put that tile up for sale. `P` and `Q` say nothing about the slope beneath them, which is the one thing this map",
            "leaves out on purpose - the paving is already there. `!` is something this map cannot name",
            "and `?` no surface could be read.",
            ...
            "instead. `area` is a view box and not a ride's footprint: it covers the margin and everything",
            "else read, and `clear_scenery` clears every tile of a rectangle it is given.",
