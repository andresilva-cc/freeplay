...
export class SiteTools {
    @mcpTool({
        ...
        description: [
            ...
            "`build_flat_ride` takes, so the three copy across unchanged.",
            ...
            "`clear_scenery`'s four arguments under the same names. That rectangle is the game's own layout and",
            "not a function of `x`, `y` and the ride's size: a square centred on the origin is the wrong ground for every",
            ...
            "`ground` is one sentence giving the whole state of the footprint — how much of it takes",
            ...
            "the entrance and one for the exit; any two of them work, and two sharing a `side` put both doors on",
            ...
            "`cost`, on every option, is one sentence giving that option's whole price — the path to",
            ...
            "from track, a rotation outside 0-3, or a tile off the map. A placement that does not",
