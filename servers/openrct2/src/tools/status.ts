...
export class StatusTools {
    @mcpTool({
        ...
        description: [
            ...
            "and a path only connects the exit if it reaches `exitDoor`: those two are the tiles a run has to",
            "cover, and the buildings are not.",
            ...
            "ride upkeep read as one number there, and the fourteen are what tell them apart. These are the",
            ...
            "warnings about the park rating. Each carries `gameDaysAgo`,",
            ...
            "reachable exactly when a run covers it — while the list is whole. `runs`, `deadEnds` and",
            "`islands` are each cut at 500 entries on the way out, with a `<N more of M omitted>` string as",
            "the last entry when that happens; past that the `tiles` come up short of `reachableTiles`, and",
            "applying the rule to a cut list calls reachable tiles unreachable.",
            "A run is a straight line and NOT a separate network:",
            ...
            "They come in the order a row-by-row scan of the map reaches them — by lowest y, then by lowest x",
            "within that row — and in no other order: not by size and not by what stands on them.",
            ...
            "and `built`. Ground the park does not own is not counted, and `blocks` is unordered: these are",
            "counts, not a ranking. At most 64 blocks come back. `complete` false means the park owns more",
            "than that and `blocks` holds 64 of them in scan order, so the six counts then add up to LESS",
            "than `owned`; while `complete` is true they add up to `owned` exactly. `block` is the side this",
            "call used and is not an argument: a park too big for 64 blocks at that side is read through",
            "view_map or evaluate instead.",
    ...
    @mcpTool({
        ...
        description: [
            ...
            "The plugin API documents that field as one thing only: the larger the number, the less",
            "fresh the thought. It says nothing about what the numbers mean, or when the game moves one.",
            "The game's own thought object carries a second field beside it, `freshTimeout`, documented as",
            "\"The freshness timeout.\" and nothing further. This tool does not read it and does not return",
            "it; `evaluate` reaches it.",
            "Nothing is dropped or discounted by either, so a complaint the game answered weeks",
