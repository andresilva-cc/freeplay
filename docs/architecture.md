...
cost it turns. `game.yml` records how the game is reached and driven.
...
plugin's MCP endpoint. The adapter's `directTools` mode registers `evaluate` as a
first-class tool with its real schema, rather than behind a discovery proxy; with a
single tool the proxy would be pure overhead.
...
result. It reaches the whole plugin API, which means it reaches every game action. That
is deliberately the entire action surface for now.

The alternative is a set of typed tools — `set_ride_price`, `hire_staff`, `open_ride` —
which is better in most respects: the schema teaches the model what is possible, invalid
calls are rejected before they run, and the tool list is a readable contract. It is also
a guess. Which typed tools to build depends on which actions a model actually reaches
for and which it fumbles, and nobody knows that yet. One general tool answers the
question; a hand-picked set of typed tools assumes the answer.
...
- **A short tool list.** Small models handle a handful of tools far better than dozens.
  One is a happy accident of the design above.
